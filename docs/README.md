...
```python
@action(
    name: str = "...", # defaults to the method name
    help: dict = {...}, # defaults to autogenerated object
    access_policy: str = ACCESS_PERMITTED,
)
...
```

The `help` attribute may be used for describing the given action whenever the
`help` action is called on the agent.
...
The following example shows how the above help information could be defined
using a docstring:
...
If a `help` object is provided, it _overwrites_ the generated object entirely.
